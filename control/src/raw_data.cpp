...
{
    return a << g.newflag << g.utc << g.pos << g.fix_quality << g.num_sats
             << g.hdop << g.alt_unit << g.undulation << g.und_unit
             << g.DGPS_age << g.DGPS_ID;
}
...
{
    return a << g.newflag << g.mode_char << g.mode_num << g.sat_prns
...
{
    return a << g.newflag << g.utc << g.pos_status << g.ground_speed
...
{
    return a << g.newflag << g.track_true << g.track_indicator << g.track_mag
...
{
    return a << g.newflag << g.num_msgs << g.msg_num << g.sats_in_view << g.sats;
}
